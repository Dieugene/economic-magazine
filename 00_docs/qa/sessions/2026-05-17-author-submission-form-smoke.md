...
## Re-test — 2026-05-18, третий прогон (после B5+B6 фикса)

Бэкендер закрыл оба блокера. Проверка:

### Прямой `curl POST` на staging
- Без zip: `HTTP 201 {"message":"Данные успешно загружены и отправлены на рецензирование"}` — B5 закрыт.
- С zip: `HTTP 201` с тем же чистым success-телом, без `error_type`/Redis-ошибки — B6 закрыт.

### UI smoke на localhost + staging-бэк (Chrome DevTools MCP)
- Заполнены: 5 подтверждений, ФИО `Иванов И.И.`, место работы, должность, город, email `qa-positive@example.com`, телефон, степень/звание/funding/ORCID, файл `qa_positive.docx` (через DataTransfer-обход), 2 согласия.
- Submit → Network: `POST /backend/articles/upload_new_article/` → **201**.
- UI: success-экран «Статья получена. Спасибо… свяжется с вами по адресу qa-positive@example.com.» + кнопка «Подать ещё одну статью».

### Итог
- B5: **закрыт**.
- B6: **закрыт**.
- Полный positive path фронта подтверждён UI'ем.
- Готовится деплой v0.1.15 (`dieugene/vte-frontend:0.1.15`).
- `error_type`-guard в `client.ts` оставлен как защитный код (срабатывать на нормальном ответе теперь не будет).
