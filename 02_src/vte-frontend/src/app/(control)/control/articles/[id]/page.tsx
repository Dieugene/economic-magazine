...
export default function ArticleFormPage({
  ...
  function buildPayload(): ArticleCreatePayload | null {
    ...
    if (xmlUrlChanged && xmlUrlTrimmed && !isSendableXmlUrl(xmlUrlTrimmed)) {
      ...
      toast.error(message, {
        // Пояснение разное по схемам, и это не придирка к слову: на СТАРОЙ
        // схеме кнопка «Сформировать XML» кладёт путь в это же поле, и
        // редактор, увидев ошибку, первым делом кидается править то, что
        // писал не он. На НОВОЙ схеме путь в это поле попасть не может — он
        // живёт в `xml_file` и показан строкой «Файл на сервере», — поэтому
        // старое пояснение объясняло бы про значение, которого тут не бывает,
        // да ещё в момент, когда человек уже ошибся. Найдено приёмкой
        // (наблюдение Obs-3).
        description:
          splitXmlFields === false
            ...
            : "Здесь ждут внешний адрес XML на сайте РЦНИ — целиком, вместе с «https://». " +
              "Файл, собранный нашим сервером, в это поле не попадает.",
      });
