...
//
// Зачем. 07.09.2026 боевой сайт лёг под нагрузкой, и замер показал, что кэша
// не было нигде: каждый показ страницы шёл в Django заново, сто читателей
// подряд на одной странице давали сто одинаковых запросов. С кэшем бэкенд
// получает один запрос в минуту на адрес, сколько бы читателей ни пришло.
//
// Почему именно 60 секунд, а не больше. Данные правят не только через нашу
// админку, но и через Django-админку на том же сервере (`/admin/`), а о её
// правках фронт узнать не может ничем. Значит предел жизни кэша — единственная
// страховка от «редакция поправила, а на сайте старое»; минуту редакция не
// замечает, четверть часа заметит и сочтёт сайт сломанным. Досрочный сброс при
// правке через нашу админку сделан отдельно — в `app/backend/[...path]/route.ts`.
const PUBLIC_CACHE_TTL = 60;

// Тег, по которому кэш сбрасывается досрочно. Один на все публичные ответы:
// правка статьи меняет и страницу номера, и страницу рубрики, и главную (списки
// выпусков отдаются с вложенными статьями), так что раздельные теги дали бы
// дыры, а выигрыша не дали бы — редакция правит редко.
export const PUBLIC_CACHE_TAG = 'public-api';

// `next` — расширение fetch от Next.js, и в типе RequestInit оно уже описано
// самим Next (свой тип поверх объявлять нельзя — не сойдётся). В браузере это
// поле движок игнорирует как нераспознанное, поэтому клиентские вызовы (поиск,
// скачивание, список рубрик в админке) от кэша не меняются.

// Опции кэширования для запроса — только для серверных публичных чтений.
//
// 🛑 Признак «публичный» здесь — `auth !== true`, то есть НАМЕРЕНИЕ вызывающего
// кода, а не наблюдение за заголовками. Смотреть на заголовок Authorization
// нельзя: `tokenStore.getAccess()` на сервере всегда возвращает null (см. выше),
// и такой признак означал бы просто «серверный запрос» — приватный ответ лёг бы
// в общий кэш под ключом без пользователя, и следующий анонимный читатель
// получил бы чужие данные. Сегодня это не стреляет только потому, что вся
// админка клиентская: **вынести любую её страницу в серверный компонент, не
// пересмотрев это место, нельзя.**
//
// ⚠️ Метод сравниваем с умолчанием: ни один публичный вызов `method` не
// передаёт, и проверка `init.method === 'GET'` не сработала бы ни разу — весь
// кэш молча остался бы выключенным.
function withPublicCache(path: string, init: RequestInit): RequestInit {
  const method = (init.method ?? 'GET').toUpperCase();
  if (!isServer || method !== 'GET') return init;
  // Второй тег — по ресурсу (`/issues/12/` → `public-api:issues`). Сбросом
  // пока не пользуемся, но он даёт возможность сбрасывать точечно, не трогая
  // остальное, если правки станут частыми.
  const resource = path.replace(/^\//, '').split(/[/?]/)[0] || 'root';
  return {
    ...init,
    next: { revalidate: PUBLIC_CACHE_TTL, tags: [PUBLIC_CACHE_TAG, `${PUBLIC_CACHE_TAG}:${resource}`] },
  };
}
...
async function fetchApi<T>(path: string, options: FetchOptions = {}): Promise<T> {
  ...
  const res = needAuth
    ...
    : await fetch(`${API_BASE}${path}`, { ...withPublicCache(path, init), headers });
