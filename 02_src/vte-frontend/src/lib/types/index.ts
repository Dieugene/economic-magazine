...
export interface IssueSummary {
  ...
  // Число рубрик: бэк считает его сам, как articles_count. Пока поля нет,
  // считаем по sections — см. комментарий к sections ниже.
  sections_count?: number;
  // ⚠️ Списочный ответ /issues/ вкладывает сюда ВСЕ статьи всех выпусков со
  // всеми аннотациями — 6,74 МБ на 31 выпуск. Публичной части эти данные из
  // списка не нужны нигде: главной хватает пяти скалярных полей, странице
  // номера — только id, архиву — года. Бэкенду отправлена просьба убрать
  // sections из списочного сериализатора, оставив в detail. Поле остаётся
  // необязательным, чтобы правка бэка ничего у нас не сломала.
